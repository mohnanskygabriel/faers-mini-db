...
public class DBImporter {
	...
	public void importJson(String json, String ip, String port, String db, String username, String password) {
		...
		try {
			...
			for (Result result : event.getResults()) {
				session.save(result.getPatient());
			}
			tx.commit();
			logger.info("Data import successfully completed from file: " + json);
			session.getTransaction().begin();
			logger.info("Deleting reaction duplicates...");
			session.createStoredProcedureQuery("faers.clear_reaction_duplicates").execute();
			session.getTransaction().commit();
			logger.info("Duplicates deleted.");
			/*
			 * logger.
			 * info("Executing VACUUM FULL on reaction and pantiet_reaction_mapping tables"
			 * ); session.createNativeQuery("VACUUM FULL faers.reaction");
			 * session.createNativeQuery("VACUUM FULL faers.patient_reaction_mapping");
	...
	public void importAllJsonFromDirectory(File sourceDirectory, String ip, String port, String db, String username,
			String password) {
